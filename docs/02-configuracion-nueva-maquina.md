...
- `Win+Alt+C` → **Nuevo cliente**: crea `Clientes\<cliente>\` (marca, docs, entregables, admin).
- `Win+Alt+N` → **Nuevo proyecto**: crea `Proyectos\<cliente>\<proyecto>\` completo.
...
proyecto posterior se crea con `Win+Alt+N`.
...
- **AutoHotkey**: el `context-manager.ahk` arranca con Windows (está en Inicio).
  ...
  | `Win+Alt+H` | Modo gamer: mata Adobe/browser/Notion/Bitwarden y avisa qué cerró |
...
- [ ] Hotkeys `Win+Alt+E` y `Win+Alt+H` funcionan.
